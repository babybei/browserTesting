...
* */
    function playBar(oPlayBar) {
        ...
        var strAudioSelector = '#' + oPlayBar.audioMp3Id.replace(/\s*,\s*/,',#');
        var audios = $(strAudioSelector), allTime,currentTime;

        audios.on('play',function () {
            var preAudio = audios.get(audios.index(this)-1);
            if(!(this === audios.get(0) || (preAudio && preAudio.ended))){
                this.pause();
            }
        }).on("loadeddata.audioBar", function () {
            allTime = 0;
            ...
            $(this).on('timeupdate',function () {
                var imgInd = oldImgInd,imgTime = 0;
                 currentTime = 0;
                audios.each(function () {
                    currentTime += (this.ended ? this.duration : this.currentTime);
                });
            ...
            if(typeof  oPlayBar.ended == "function"){
                $(this).on("ended", function(){
                    if(this === audios.get(-1)){
                        oPlayBar.ended.call(this,this)
                    }
                    else {
                        audios.get(audios.index(this)+1).play()
                    }
                });
            }
        }).filter(function(){
